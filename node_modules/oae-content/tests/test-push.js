    ...
    describe('Notifications', function() {
        ...
        it('verify a new revision triggers a push notification', function(callback) {
            setupFixture(function(contexts, contentObj, client) {
                ...
                client.on('message', function(message) {
                    ActivityTestsUtil.assertActivity(message.activities[0], ContentConstants.activity.ACTIVITY_CONTENT_REVISION, ActivityConstants.verbs.UPDATE, contexts['branden'].user.id, contentObj.id);
                    ...
                    RestAPI.Content.getContent(contexts['branden'].restContext, contentObj.id, function(err, contentObj) {
                        assert.ok(!err, JSON.stringify(err));
                        assert.equal(message.activities[0].object.latestRevisionId, contentObj.latestRevisionId);
        ...
        it('verify restoring a revision triggers a push notification', function(callback) {
            setupFixture(function(contexts, contentObj, client) {
                ...
                RestAPI.Content.updateFileBody(contexts['branden'].restContext, contentObj.id, getFileStream, function(err) {
                    ...
                    client.on('message', function(message) {
                        if (message.resourceId === contentObj.id && message.streamType === 'activity' && message.activities[0]['oae:activityType'] === ContentConstants.activity.ACTIVITY_CONTENT_RESTORED_REVISION) {
                            ActivityTestsUtil.assertActivity(message.activities[0], ContentConstants.activity.ACTIVITY_CONTENT_RESTORED_REVISION, ActivityConstants.verbs.UPDATE, contexts['branden'].user.id, contentObj.id);
        ...
        it('verify a new comment triggers a push notification', function(callback) {
            setupFixture(function(contexts, contentObj, client) {
                ...
                client.on('message', function(message) {
                    ActivityTestsUtil.assertActivity(message.activities[0], ContentConstants.activity.ACTIVITY_CONTENT_COMMENT, ActivityConstants.verbs.POST, contexts['branden'].user.id, comment.id, contentObj.id);
